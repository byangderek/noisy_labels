...
def train():
    # Import data
    train = mnist('train', noise_type=2, noise_ratio=0.3, noise_prior=0.5, is_train=True)
    val = mnist('val')
    test = mnist('test')
        ...
        with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
            ...
            def feed_dict(dataset, batch_size, train=False):
                ...
                if train:
                    ...
                    k = 1.0
                return {x: xs, y_: ys, keep_prob: k, m: np.log(dataset._noise_matrix)}

            sess.run(tf.assign(m_param, m), feed_dict=feed_dict(train, 128, True))
