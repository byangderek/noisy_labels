...
def train():
    # Import data
    train = mnist('train', noise_type=2, noise_ratio=FLAGS.noise_ratio, use_init=True, is_train=True)
    val = mnist('val')
    test = mnist('test')
    ...
    with tf.Graph().as_default():
        ...
        m_param = weight_variable([10,10])
        m_matrix = tf.nn.softmax(m_param)
        y2 = tf.matmul(y1, m_matrix)
        ...
        with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
            ...
            def feed_dict(dataset, batch_size):
                ...
                return {x: xs, y_: ys, m: np.log(dataset._noise_matrix)}

            sess.run(tf.assign(m_param, m), feed_dict=feed_dict(train, 128))
            ...
            # save learned noise model parameter
            m = sess.run(m_matrix, feed_dict=feed_dict(test, 0))
            np.save('weights/lr_m_%.1f_learn.npy' % FLAGS.noise_ratio, m)
