...
def train():
    # Import data
    train = mnist('train', noise_type=2, noise_ratio=0.7, is_train=True)
    val = mnist('val')
    test = mnist('test')
        ...
        with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
            ...
            for i in range(FLAGS.max_steps):
                if i % 200 == 0:  # Record summaries and test-set accuracy
                    ...
                    loss, acc, _ = sess.run([cross_entropy, accuracy, train_step], feed_dict=feed_dict(train, FLAGS.batch_size, True))
                    if i % 20 == 0:
